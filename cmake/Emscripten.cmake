...
set(exploration_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(exploration_COI_WORKER "${exploration_WEB_DIR}/coi-serviceworker.min.js")
set(exploration_SHELL_TEMPLATE "${exploration_WEB_DIR}/shell_template.html.in")
set(exploration_INDEX_TEMPLATE "${exploration_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  # Set WASM build flag
  set(exploration_WASM_BUILD ON CACHE BOOL "Building for WebAssembly" FORCE)

  # Sanitizers don't work with Emscripten
  foreach(sanitizer ADDRESS LEAK UNDEFINED THREAD MEMORY)
    set(exploration_ENABLE_SANITIZER_${sanitizer} OFF CACHE BOOL "Not supported with Emscripten")
  endforeach()

  # Disable static analysis and strict warnings for Emscripten builds
  foreach(option CLANG_TIDY CPPCHECK WARNINGS_AS_ERRORS)
    set(exploration_ENABLE_${option} OFF CACHE BOOL "Disabled for Emscripten")
  endforeach()
  ...
  set(exploration_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(exploration_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(exploration_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(exploration_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY exploration_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY exploration_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY exploration_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY exploration_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE exploration_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${exploration_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${exploration_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${exploration_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${exploration_SHELL_TEMPLATE}")
      configure_file(
        "${exploration_SHELL_TEMPLATE}"
        "${CONFIGURED_SHELL}"
        @ONLY
      )
      ...
      message(FATAL_ERROR "Shell template not found: ${exploration_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${exploration_COI_WORKER}")
      add_custom_command(TARGET ${target} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
          "${exploration_COI_WORKER}"
...
function(exploration_create_web_dist)
  if(NOT EMSCRIPTEN)
    return()
  endif()
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY exploration_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY exploration_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY exploration_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${exploration_INDEX_TEMPLATE}")
    configure_file("${exploration_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${exploration_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY exploration_WASM_TARGET_${target}_OUTPUT_NAME)
